# örnek kullanıcı girdisinin alındığı kısım  dosya yapısı
...
# Araç Listeleme ve Etkileşim Döngüsü
while True:
    print("---------------------------------- Tüm Araçlar ------------------------------------")
    ...
    # Kullanıcıdan ID seçimi
    while True:
        try:
            secim_id = int(input("Durumunu görmek istediğiniz aracın ID'sini girin (çıkış için 0): "))
            if secim_id == 0:
    ...
    # Seçilen her araca göre davranış
    bitirilecek_arac_id = None
    ...
    if isinstance(secilen_arac, (Scooter, Bisiklet)):
        ...
        bitirilecek_arac_id = secilen_arac.arac_id  # sefer bitirme işlemini rapordan sonra yapacağız
        ...
        bitirilecek_arac_id = secilen_arac.arac_id
        ...
        # Shuttle için de sefer geçmişine ekleme
        service.sefer_baslat(sefer_id=300 + secilen_arac.arac_id, arac_id=secilen_arac.arac_id)
        bitirilecek_arac_id = secilen_arac.arac_id

    # Aracın kapasite ve durum bilgisini verir (sefer bitirmeden önce raporla)
    print("\n-------------------------- Araç Kapasite ve Durum Bilgisi -------------------------------")
    for arac in yonetici.araclar:
        bilgi = arac.bilgi_ver()
        print(
            f"ID: {arac.arac_id} | Tip: {bilgi['tip']} | Durum: {bilgi['durum']} | "
    ...
    # Seferi raporlamadan sonra bitir (sıfırlamalar bundan sonra olacak)
    if bitirilecek_arac_id is not None:
        yonetici.sefer_bitir(bitirilecek_arac_id)
        print(f"\nSefer bitirildi: Araç ID {bitirilecek_arac_id}")
